...
class KumbiaActiveRecord
{
    ...
    public function save($values=null)
    {
        ...
        foreach ($this->not_null as $f) {
            ...
            if (!isset($this->$f) || is_null($this->$f) || $this->$f == '') {
                ...
                Flash::error("h2", "Error: El campo $f no puede ser nulo");
                return false;
            }
        }
            ...
            foreach ($this->fields as $field) {
                if ($field != $this->primary_key[0] || $this->{$this->primary_key[0]}) {
                    if (in_array($field, $this->_at)) {
                        if ($config['type'] == 'oracle') {
                            //$this->$field = date("Y-m-d"); // MODIFICADO POR JAVIER SALA XQ ME TOMABA LA FECHA Y HORA AUTOMATICAMENTE SIN IMPORTAR LO QUER PONIA EN EL INPUT DEL FORMULARIO EN LA VISTA. POR EJEMPLO: EN EL FORMULARIO PONIA 2020-01-30 10:00:00 Y ME REGISTRABA FECHA Y HORA ACTUAL
                        } else {
    ...
    function update()
    {
        ...
        if ($this->exists()) {
            ...
            Flash::error('h1', 'No se puede actualizar porque el registro no existe');
            return false;
        }
    }
