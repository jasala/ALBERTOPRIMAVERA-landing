...
abstract class Upload {
    ...
    public function isUploaded() {
        ...
        if ($_FILES[$this->_name]['error'] > 0) {
            ...
            Flash::error("h2", 'Error: ' . $error[$_FILES[$this->_name]['error']]);
            return FALSE;
        }
        return TRUE;
    }
    ...
    protected function _overwrite($name) {
        ...
        if (file_exists("$this->_path/$name")) {
            Flash::error("h2", 'Error: ya existe este fichero. Y no se permite reescribirlo');
            return FALSE;
        }
        return TRUE;
    }
    ...
    protected function _toBytes($size) {
        ...
        switch ($tipo) {
            ...
            default:
                $size = -1;
                Flash::error("h2", 'Error: el tamaño debe ser un int para bytes, o un string terminado con K, M o G. Ej: 30k , 2M, 2G');
        }
